...
export async function search(query: string, count: number, order: Order = "relevance", signal?: AbortSignal): Promise<SearchApiResult> {
...
export async function video(ids: string[], signal?: AbortSignal): Promise<VideoApiResult> {
  const body = new URLSearchParams({
    key,
    id: ids.join(","),
